...
function ConvertCSVFile2XMLFile()
{
	...
	do
		...
		NameRecord1=`echo "${CSVLine}" | awk -F',' {'print$1'}`
		NameRecord2=`echo "${CSVLine}" | awk -F',' {'print$2'}`
		NameRecord3=`echo "${CSVLine}" | awk -F',' {'print$3'}`
		NameRecord4=`echo "${CSVLine}" | awk -F',' {'print$4'}`

		echo -e '\t<Name>'${NameRecord1} ${NameRecord2} ${NameRecord3} ${NameRecord4}'</Name>' >> ${XMLFileName}

		if [ ${PBXDialing:-test} = "true" ]
		then
			echo -e '\t<Telephone>'$(printf "${CSVLine}" | awk -F',' {'print$8'})'</Telephone>' >> ${XMLFileName}
		fi

		if [ ${IPDialing:-test} = "true" ]
		then
			echo -e '\t<Telephone>'$(printf "${CSVLine}" | awk -F',' {'print$9'})'</Telephone>' >> ${XMLFileName}
		fi
